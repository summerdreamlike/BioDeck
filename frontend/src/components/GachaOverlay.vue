...
function runEnter(){
  ...
  }, Math.max(80, 260 - props.results.length * 18))
  // 翻面节奏
  let k = 0
...
.panel{ position:relative; width:min(980px, 92vw); max-height: 86vh; background: #fff; border-radius:16px; padding:16px; box-shadow: 0 24px 80px rgba(0,0,0,.25); transform: translateY(12px) scale(.98); opacity:0; transition: transform .36s cubic-bezier(.22,.61,.36,1), opacity .36s; display:flex; flex-direction:column; }
...
.cell{ position:relative; border-radius:12px; padding:10px; background:#eef2f7; box-shadow: inset 0 0 0 1px rgba(0,0,0,.06); transform: translateY(8px); opacity:0; transition: transform .35s, opacity .35s; }
...
.card{ position:relative; height:96px; border-radius:10px; transform-style: preserve-3d; transition: transform .6s cubic-bezier(.22,.61,.36,1); box-shadow: 0 8px 18px rgba(0,0,0,.06), inset 0 0 0 1px rgba(0,0,0,.06); }
...
.card .beam{ position:absolute; left:50%; bottom:-10%; width:24%; height:140%; transform: translateX(-50%); pointer-events:none; background: linear-gradient(to top, rgba(253,187,45,.0), rgba(253,187,45,.7), rgba(253,187,45,0)); filter: blur(4px); animation: beamUp 1.6s ease forwards; }
